...
namespace AceLand.Lifecycle
{
    ...
    internal static class FrameScheduler
    {
        ...
        /// <summary>
        /// Diagnostic push channel for the PlayerLoop editor window. Raised on the main thread the moment
        /// a handle enters the error state, immediately before the handle is removed from its bucket.
        /// Runtime keeps no error state of its own: if no window is listening the error simply is not
        /// retained (the exception is still logged via <see cref="LifecycleLog"/>).
        /// </summary>
        internal static event Action<FrameHandle> ProcessErrored;
        ...
        private static void OnTick(PlayerLoopPoint point)
        {
            ...
            for (var i = bucket.Count - 1; i >= 0; i--)
            {
                ...
                // Errored handles are pushed to the editor window (if any) and removed immediately;
                // runtime never lingers on an error node of its own.
                if (handle.State == FrameProcessState.Error)
                {
                    try
                    {
                        ProcessErrored?.Invoke(handle);
                    }
                    catch (Exception e)
                    {
                        LifecycleLog.Exception(e);
