...
namespace AceLand.Lifecycle
{
    ...
    public static class ModuleRegistry
    {
        ...
        private static async Task RunPhaseInternal(ModulePhase phase)
        {
            ...
            var issuesBefore = issues.Count;
            var sorted = ModuleSorter.Sort(batch, byId, issues);
            // The sorter is side-effect free — it only records findings. Surface any newly
            // discovered issues (e.g. circular dependencies) here so they reach the Console.
            for (var i = issuesBefore; i < issues.Count; i++)
                LifecycleLog.Error(issues[i]);
