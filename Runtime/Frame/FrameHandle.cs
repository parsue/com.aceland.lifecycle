...
namespace AceLand.Lifecycle
{
    ...
    /// window can draw it as a node. A handle whose action threw is flagged as errored and removed on the
    /// same tick; the scheduler pushes it to the editor window (if any) beforehand and its awaiter is
    /// resolved (cancelled) immediately. Runtime keeps no error state of its own.
    /// </para>
    /// </summary>
    internal sealed class FrameHandle : IFrameHandle
    {
        ...
        /// <summary>
        /// Resolves the awaiter immediately (cancelled) and flags the handle as errored. The scheduler
        /// pushes the errored handle to the editor window (if any) and removes it on the same tick.
        /// </summary>
